...
def get_app_version():
    """
    Calculates a version number based on the file's modification date.
    Starting at 1.01 (Jan 29), it increments by .01 for every day since launch.
    """
    start_date = datetime(2026, 1, 29)
    current_date = datetime.now()
    days_since = (current_date - start_date).days
    version = 1.01 + (days_since * 0.01)
    ...
    return f"{version:.2f}", last_updated

VERSION, UPDATED_TIME = get_app_version()
...
def calculate_equity(hero_str, board_str, num_opps, sims=1000):
    ...
    # FIXED: Proper Hand Type Detection
...
def get_pro_heatmap():
    """Professional 169-hand matrix (Suited/Offsuit/Pairs)."""
    ...
    return grid.style.background_gradient(cmap='RdYlGn', axis=None, gnumeric=data)\
        .set_properties(**{'text-align': 'center', 'font-size': '12px', 'height': '35px'})
...
status = p_col2.radio("Situation", ["Checked to Me", "Facing a Bet"])
call_bb = p_col3.number_input("To Call (bb)", 0.0, 500.0, 5.0) if status == "Facing a Bet" else 0.0
...
if st.button("🔥 RUN GTO ANALYSIS"):
    ...
    s1, s2 = st.columns(2)
    with s1:
        st.write(f"**Current Strength:** You have a **{hand_type}**.")
        st.write(f"**Game State:** Facing **{num_opps}** opponent(s) in a {status.lower()} pot.")
    with s2:
        if status == "Facing a Bet":
            st.write(f"**Math:** You need **{odds:.1f}%** equity to call. Your **{equity:.1f}%** makes this a {decision}.")
        else:
            st.write(f"**Strategy:** Since you have {equity:.1f}% equity, the optimal play is to {decision}.")

# --- AUTO-STAMP ---
st.markdown(f"<br><hr><center><p style='color: gray; font-size: 10px;'>v{VERSION} | Live Since: {UPDATED_TIME}</p></center>", unsafe_allow_code=True)
