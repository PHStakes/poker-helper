...
def analyze_texture(board_strs):
    ...
    try:
        # Convert to treys objects for analysis
...
with col1:
    ...
    board_list = []
    # Logic: Board disappears if Pre-flop is selected
    if street != "Pre-flop":
...
if st.button("Get Pro Recommendation"):
    with st.spinner("Analyzing math..."):
        # We pass a copy of board_list to avoid modifying the UI list
    ...
    # Hide texture and bluff logic if Pre-flop
    if street != "Pre-flop":
    ...
    if action == "Facing a Bet":
        ...
        if street == "Pre-flop":
            ...
            if equity > 65:
                # Automatic sizing based on texture analysis
                texture_label, is_wet = analyze_texture(board_list)
