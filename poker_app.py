...
def parse_cards(raw_input):
    ...
    for c in cards:
        if len(c) == 2:
            rank = c[0].upper()  # Corrected line: no spans or ghost tags
            suit = c[1].lower()
...
def calculate_equity(hero_str, board_str, num_opps, sims=1000):
    ...
    for _ in range(sims):
        ...
        # Fixed: Consistent variable name 'best_opp_score' to avoid NameError
...
def get_opponent_heatmap():
    """Restores the visual range heatmap using standardized map function."""
    ranks = ['A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2']
    grid = pd.DataFrame(0.1, index=ranks, columns=ranks)
    # Using .map instead of .applymap to avoid FutureWarnings found in logs
...
if st.button("🔥 RUN GTO ANALYSIS"):
    ...
    # Simple logic to determine number of opponents from action string
    num_opps = 1
    ...
    if status == "Facing a Bet":
        ...
        if equity > odds + 10: st.success("## ✅ RAISE (3x)"); st.write("Large equity advantage.")
        elif equity > odds: st.warning("## ✅ CALL"); st.write("Profitable based on pot odds.")
        else: st.error("## ❌ FOLD"); st.write("Equity too low for this price.")
    else:
