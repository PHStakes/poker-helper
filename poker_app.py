...
def get_app_info():
    """
    Increments version based on date and pulls live timestamp from the file system.
    """
    # Version logic: 1.01 + 0.01 for every day since launch (Jan 29)
    start_date = datetime(2026, 1, 29)
    days_since = (datetime.now() - start_date).days
    auto_version = 1.01 + (max(0, days_since) * 0.01)
    
    # Live timestamp from file modification time
    try:
...
def calculate_equity(hero_str, board_str, num_opps, sims=1000):
    ...
    # Correct Hand Type Identification
...
def get_pro_heatmap():
    """Generates standard 169-hand matrix (Suited/Offsuit/Pairs)."""
    ...
    # Fixed background_gradient by using gmap instead of gnumeric
...
if st.button("🔥 RUN GTO ANALYSIS"):
    ...
    s1, s2 = st.columns(2)
    with s1:
        st.write(f"**Current Holding:** You have a **{hand_type}**.")
        st.write(f"**Field Strength:** Up against **{num_opps}** active opponent(s).")
    with s2:
        if status == "Facing a Bet":
            st.write(f"**The Math:** You need **{odds:.1f}%** equity to call. You have **{equity:.1f}%**.")
        st.write(f"**GTO Strategy:** {decision} is the most profitable play.")
