...
def get_app_info():
    """
    Increments version based on file modification date.
    Calculates 1.01 + 0.01 for every day since launch (Jan 29).
    """
    ...
    # Get last modification time of this file automatically
    try:
...
def get_pro_heatmap():
    """Standard 169-hand poker grid with suited/offsuit distinction."""
...
if st.button("🔥 RUN GTO ANALYSIS"):
    ...
    s1, s2 = st.columns(2)
    with s1:
        st.write(f"**Current Holding:** Your hole cards make a **{hand_type}**.")
        st.write(f"**Opponents:** You are up against **{num_opps}** active player(s).")
    with s2:
        if status == "Facing a Bet":
            st.write(f"**Pot Odds:** You need **{odds:.1f}%** equity to call. You have **{equity:.1f}%**.")
        st.write(f"**GTO Verdict:** {decision} is the most profitable long-term play.")

# --- AUTO-STAMP (Fixed TypeError) ---
